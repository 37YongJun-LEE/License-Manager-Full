...
<script>
	...
	function logout(){
		if(confirm("로그아웃 하시겠습니까?")){
			location.href = "/kabadm/actionLogout.do";
		}
	}
	...
	<div class="menu_wrap">
		...
		<c:choose>
			<c:when test="${usrLevelCdChk eq 'Y'}">
				<h1 class="logo"><a href="/kabadm/deliberate/reqList.do"><img src="/_img/adm/comn/logo.png" alt="한국인정지원센터 스마트인정시스템"/></a></h1> <!--img 293px*27px-->
			</c:when>
			<c:otherwise>
				<h1 class="logo"><a href="/kabadm/user/userList.do"><img src="/_img/adm/comn/logo.png" alt="한국인정지원센터 스마트인정시스템"/></a></h1> <!--img 293px*27px-->
			</c:otherwise>
		</c:choose>	
	</div> 

	<p class="gnb">
		<a href="/kabadm/user/userInfo.do?usrIdx=${userSessionVo.usrIdx}" class="my"><img class="account" src="/_img/adm/comn/my.png" alt="나의설정"/><span>${userSessionVo.usrNmKr}(${userSessionVo.usrId})</span>님 안녕하세요.</a>
...
function getNoticeList(){
	...
	chkGetNoticeAjax = 1;
	var usrIdx = '${userSessionVo.usrIdx}';
