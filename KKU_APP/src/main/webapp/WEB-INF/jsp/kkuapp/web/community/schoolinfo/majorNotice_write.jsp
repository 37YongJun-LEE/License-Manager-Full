...
<body>
	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="board head_mg mb140">
			<div class="navi">
				<c:if test="${majorNoticeInfo == null}">
					<div class="top_navi"><a href="majorNotice.do?usrIdx=${usrIdx}"><img src="/_img/_img/cont/navi_arrow.png"></a>
					<h2>학과 공지 글쓰기</h2>
				</c:if>
				<c:if test="${majorNoticeInfo != null}">
					<div class="top_navi"><a href="majorNotice_view.do?boardIdx=${majorNoticeVO.boardIdx}"><img src="/_img/_img/cont/navi_arrow.png"></a>
					<h2>학과 공지 수정하기</h2>
				</c:if>	
			</div>
		</div>
			...
			<form name="frm" id="frm" autocomplete="off" method="post" enctype="multipart/form-data">
				<input type="hidden" name="boardIdx" value="${majorNoticeInfo.boardIdx }"/>
				<input type="hidden" name="boardDetailname" value="${majorNoticeInfo.boardDetailname }"/>
				<input type="hidden" name="mode" id="mode" />
				
				<input type="text" id="boardTitle" name="boardTitle" placeholder="게시물 제목 입력" value="${majorNoticeInfo.boardTitle}" class="mb10"/>
				<textarea rows="10" id="boardContent" name="boardContent" placeholder="게시물 내용 입력" >${majorNoticeInfo.boardContent}</textarea>
				...
				<div class="add_photo">
					...
					<a href="javascript:openFile();" class="add">
						...
						<c:if test="${majorNoticeInfo != null }">
							<span class="count" id="count"><b>${majorNoticeInfo.fileCnt }</b> /5</span>
						</c:if>	
					</a>
				...
				<p class="btn mt25">
					<c:if test="${majorNoticeInfo == null }">
						<a href="javascript:submitForm('reg');" class="btn">등록</a>
					</c:if>
					<c:if test="${majorNoticeInfo != null }">
						<a href="javascript:submitForm('mod');" class="btn">저장</a>
					</c:if>
				</p>
			</div>
	...
	<div class="layer" id="ok_Pop">
		...
		<div class="pop-layer" id="okPop">
			<div class="pop_cont text">
				...
				<p class="btn"><a href="majorNotice.do?usrIdx=${usrIdx}">확인</a></p>
			</div> 
		</div>
	</div>
