	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="board head_mg mb140">
			...
			<form name="frm" id="frm" autocomplete="off" method="post" enctype="multipart/form-data">
				<p class="path mb10" id="bname">자유게시판</p>
					<input type="hidden" name="boardIdx" value="${BoardListVO.boardIdx}"/>
				 <input type="hidden" name="boardName" value="Cboard"/>
				 <input type="hidden" name="boardDetailname" value="자유게시판"/>
				<input type="text" id="boardTitle" name="boardTitle" placeholder="게시물 제목 입력" class="mb10" value="${BoardListVO.boardTitle}"/>
				<textarea rows="10" id="boardContent" name="boardContent" placeholder="게시물 내용 입력" value="${BoardListVO.boardContent}">${BoardListVO.boardContent}</textarea>
				<div class="add_photo">
					<input type="file" class="file" name="input_usr_file" id="input_usr_file" accept="image/*" onchange="addFile(this);" style="display: none" multiple/>
						<a href="javascript:openFile();" class="add">
							...
							<c:if test="${BoardListVO == null }">
								<span class="count" id="count"><b>0</b> /1</span>
							</c:if>
							<c:if test="${BoardListVO != null }">
								<span class="count" id="count"><b>${BoardListVO.fileCnt }</b> /1</span>
							</c:if>	
						</a>
					<ul class="file_wrap">
							<c:forEach var="file" items="${fileList }">
									...
									-->
										<span><img src="/_img/_img/cont/photo_thum.png" width="65" height="65" style="background:url(/upload/board/${file.fileSrvFile })no-repeat 50% 50% / cover"></span>
