...
function report(idx, boardYN){
	if(boardYN == 'Y'){
		var parm ="/declar.do?reportBoardIdx="+idx+"&reportTblCode=10";
		location.href=parm;
	}
	else if(boardYN == 'N'){
		var parm ="/declar.do?reportBoardIdx="+idx+"&reportTblCode=10&reportPageurl=reply";
		location.href=parm;
	}
	...
	function likeButton() {
		var boardIdx = "${boardDetail.boardIdx}";
		var usrId = "${UsrID}";
		var boardDetailname = "${boardDetail.subjectName}";
		var boardName = "${boardDetail.subjectCd}";
		$.ajax({
			contentType : "application/x-www-form-urlencoded; charset=UTF-8",
			type : "POST",
			url : "/boardLike.do",
	...
	function replyWriteAjax() {
		var boardIdx = "${boardDetail.boardIdx}";
		var boardWriter = "${boardDetail.boardWriter}";
		var boardTitle = "${boardDetail.boardTitle}";
		var boardDetailname = "${boardDetail.subjectName}";
		var boardName = "${boardDetail.subjectCd}";
...
<body>
	...
	<%@include file="../_inc/header.jsp" %>
		...
		<div id="sub_contents" class="board head_mg mb140">
			...
			<div class="bbs_view">
				<div class="view_top">
					<p class="tit">${boardDetail.boardTitle}</p>
					<div class="writer">
						<a href="#" class="photo"> <span> <c:choose>
									<c:when test="${boardDetail.usrSrvprofilepic ne null }">
										<p>
											<img src="/filePathlicenseBoard/${boardDetail.usrSrvprofilepic}">
										</p>
									</c:when>
									<c:otherwise>
						...
						<div class="info">
							...
							<a href="#" class="name"> ${boardDetail.boardWriter } </a>
							<%-- <p class="depart">
							...
							<p>
								...
								<span>조회수 ${boardDetail.boardViewnum }</span>
							</p>

						</div>
						
							<p class="declar"><a href="#" onclick="report('${boardDetail.boardIdx}','Y')">신고</a></p>
					</div>
				</div>
				<div class="view_bottom">
						...
						<p class="img">
							<c:forEach var="file" items="${fileList }">
								<c:if test="${file.bfType == 'img' }">
									<img alt="자유게시판" src="/upload/licenseBoard/${file.fileSrvFile }" >
								</c:if>
							</c:forEach>
						</p> 
					</div>
					...
					</span> 이 게시물 추천<strong>${likeCheck}</strong> </a>
					<p class="btn nk_btn mt15">
						<a href="licenseBoard.do?subjectCd=${boardDetail.subjectCd}" class="bt_greenL">목록</a>

						<c:if test="${boardDetail.boardWriter eq UsrID }">
							<a
								href="licenseBoardModify.do?boardIdx=${boardDetail.boardIdx}&subjectCd=${boardDetail.subjectCd}"
				...
				<ul>
					...
					<c:forEach var="result" items="${replyList }">
						...
						<li>
							...
							<p class="declar"><a href="#" onclick="report('${boardDetail.boardIdx}','N')">신고</a></p>
							</c:if>
							</p>
		...
		<div class="layer" id="del_Pop">
			...
			<div class="pop-layer" id="delPop">
				<div class="pop_cont text">
					...
					<p class="two_btn btn">
						<a href="#" onclick="deleteAction('${boardDetail.boardIdx}')">확인</a><a
		...
		<div class="layer" id="delok_Pop">
			...
			<div class="pop-layer" id="delokPop">
				<div class="pop_cont text">
					...
					<p class="btn">
						<a href="/licenseBoard.do?subjectCd=${boardDetail.subjectCd}">확인</a>
					</p>
				</div>
			</div>
		</div>
