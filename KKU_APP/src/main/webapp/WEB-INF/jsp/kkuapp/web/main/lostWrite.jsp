...
<body>
	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="info head_mg mb140">
			...
			<div class="navi">
				<div class="top_navi"><a href="/lost.do"><img src="/_img/_img/cont/navi_arrow.png"></a>
					<c:if test="${lostInfo == null }">
						<h2>분실물 글쓰기</h2>
					</c:if>	
					<c:if test="${lostInfo != null }">
						<h2>분실물 수정하기</h2>
			...
			<div class="bbs_write w_box top no_line">
				<form name="frm" id="frm" autocomplete="off" method="post" enctype="multipart/form-data">
					...
					<input type="text" id="lostTitle" name="lostTitle" placeholder="제목" class="mb10" value="${lostInfo.lostTitle }"/>
					
					<select class="path mb10" id="lostSelecter" name="lostSelecter">
						<c:if test="${lostInfo != null }">	
							<option selected="selected" hidden>${lostInfo.lostSelecter }</option>
						</c:if>					
						<option>분실물 찾아가세요</option>
						<option>분실물 찾습니다</option>
					</select>		
					
					<input type="text" id="lostLocation" name="lostLocation" placeholder="습득 / 분실 장소 입력"" class="mb10" value="${lostInfo.lostLocation }"/>
					
					
					<textarea type="text" rows="7" id="lostContent" name="lostContent" placeholder="내용을 입력해주세요.">${lostInfo.lostContent }</textarea>
					...
					<div class="add_photo">
						...
						<a href="javascript:openFile();" class="add">
							...
							<c:if test="${lostInfo != null }">
								<span class="count" id="count"><b>${lostInfo.fileCnt }</b> /5</span>
							</c:if>	
						</a>
						...
						<ul class="file_wrap">
							<c:forEach var="file" items="${fileList }">
									...
									-->
										<span><img src="/_img/_img/cont/photo_thum.png" width="65" height="65" style="background:url(/upload/lost/${file.fileSrvFile })no-repeat 50% 50% / cover"></span>
	...
	<div class="layer" id="ok_Pop">
		...
		<div class="pop-layer" id="okPop">
			<div class="pop_cont text">
				...
				<p class="btn"><a href="/lost.do">확인</a></p>
			</div> 
		</div>
	</div>
		...
		<div class="pop-layer" id="faildPop">
			...
			<div class="pop_cont text" id="two">
				<p class="mb20 c">습득 / 분실 장소를 입력하세요.</p>
				<p class="btn"><a href="#" class="cbtn">확인</a></p>
			</div> 
		</div>
	</div>
