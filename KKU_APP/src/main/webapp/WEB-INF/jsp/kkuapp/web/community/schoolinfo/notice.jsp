	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="board head_mg mb140">
			...
			<div class="navi small">
				<div class="top_navi"><a href="/main.do"><img src="/_img/_img/cont/navi_arrow.png"></a><h2>학교 공지</h2></div>
			</div>
			...
			<form name="schFrm" id="schFrm" method="post" action="<c:url value='/notice.do?noticeCategory=${vo.noticeCategory}'/>">
				...
				<div class="list_top sub_tab">
					<p> <!-- 해당되는 메뉴에 클래스 on을 붙여주세요 -->
						<a href="notice.do?noticeCategory=학사공지" <c:if test="${vo.noticeCategory eq '학사공지'}">class="on"</c:if>>학사공지</a>
						<a href="notice.do?noticeCategory=장학공지" <c:if test="${vo.noticeCategory eq '장학공지'}">class="on"</c:if>>장학공지</a>
						<a href="notice.do?noticeCategory=취업/창업" <c:if test="${vo.noticeCategory eq '취업/창업'}">class="on"</c:if>>취업/창업</a>
						<a href="notice.do?noticeCategory=국제교류" <c:if test="${vo.noticeCategory eq '국제교류'}">class="on"</c:if>>국제교류</a>
						<a href="notice.do?noticeCategory=일반공지" <c:if test="${vo.noticeCategory eq '일반공지'}">class="on"</c:if>>일반공지</a>
						<a href="notice.do?noticeCategory=외부행사/공모전" <c:if test="${vo.noticeCategory eq '외부행사/공모전'}">class="on"</c:if>>외부행사/공모전</a>
					</p>
				</div>
				...
				<c:choose>
					<c:when test="${noticeList ne null }">
						<c:forEach var="notice" items="${noticeList}">
							<li onclick="location.href='notice_view.do?noticeIdx=${notice.noticeIdx}'">
								<div>
									<div>
									...
									<p class="bottom">
										...
										<span>조회수 ${notice.noticeViewcnt }</span>
									</p>
								</div>
							</li>
						</c:forEach>
