	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="board head_mg mb140">
			...
			<div class="navi small">
				<div class="top_navi"><a href="javascript:history.back();"><img src="/_img/_img/cont/navi_arrow.png"></a><h2>전체 인기글 게시판</h2></div>
			</div>
			<div class="w_box no_line">
				 <form name="schFrm" id="schFrm" method="get" autocomplete="off" action="board.do">
				...
				<c:choose>
					...
					<c:forEach var="result" items="${BoardList}">
						<c:choose>
							...
							<c:otherwise>
								<li onclick="location.href='themeboard_view.do?boardIdx=${result.boardIdx}'">
									<div>
										<div>
												<p class="tit">#${result.boardDetailname }</p>
 											<p class="con">${result.boardContent }</p>
										</div>
										<p class="bottom">
											<span>${result.usrNickname }</span>
