...
function submit(){
    ...
    isRun = true;
	if( $("#tboardContent").val() !=null){
	    loading("글을 등록 중입니다...");
		$.ajax({
			type : "POST",
			url : "/themeboardModifyAction.do",
			data: formData,
	...
	<!-- CONTENTS BEGIN -->
		<div id="sub_contents" class="board head_mg mb140">
			...
			<div class="bbs_write w_box top no_line">
				...
				<p class="path mb10">${tboardVO.tboardDetailname }</p>
				<input type="hidden" name="tboardIdx" value="${tboardVO.tboardIdx }"/>
				 <input type="hidden" name="bthemeName" value="${tboardVO.tboardDetailname }"/>
				  <input type="hidden" name="tboardDetailname" value="${tboardVO.tboardDetailname }"/>
				<textarea rows="10" name="tboardContent" id="tboardContent" placeholder="게시물 내용 입력"/>${tboardVO.tboardContent }</textarea>
				<div class="add_photo">
						...
						<a href="javascript:openFile();" class="add">
							...
							<c:if test="${tboardVO == null }">
								<span class="count" id="count"><b>0</b> /1</span>
							</c:if>
							<c:if test="${tboardVO != null }">
								<span class="count" id="count"><b>${tboardVO.fileCnt }</b> /1</span>
							</c:if>	
						</a>
						...
						<ul class="file_wrap">
							<c:forEach var="file" items="${fileList }">
									...
									-->
										<span><img src="/_img/_img/cont/photo_thum.png" width="65" height="65" style="background:url(/upload/tboard/${file.fileSrvFile })no-repeat 50% 50% / cover"></span>
	...
	<div class="layer" id="ok_Pop">
		...
		<div class="pop-layer" id="okPop">
			<div class="pop_cont text">
				...
				<p class="btn"><a href="/themeboard.do?bthemeName=${ tboardVO.tboardDetailname}">확인</a></p>
			</div> 
		</div>
	</div>
	<!-- POPUP END -->
